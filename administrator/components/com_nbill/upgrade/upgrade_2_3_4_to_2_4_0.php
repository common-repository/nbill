...
INDEX ( `source_pk` )
    ...
    if ($pending_posts)
    {
        ...
        {
            if (nbf_common::nb_password_decrypt($pending_post->posted_values) == $pending_post->posted_values)
            {
                $encrypted = nbf_common::nb_password_encrypt($pending_post->posted_values);
                if (strlen($encrypted) > strlen($pending_post->posted_values))
                {
                    $sql[] = "UPDATE #__nbill_pending_orders SET posted_values = '" . nbf_common::nb_password_encrypt($pending_post->posted_values) . "' WHERE id = " . intval($pending_post->id);
                }
            }
        }
    }
