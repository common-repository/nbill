	...
	public static function showFiles($path, $files, $attachments = array())
	{
        ...
        foreach ($files as $file)
        {
            if (!is_dir($current_path . "/" . $file['name']))
            {
                ...
                <tr>
                    <td class="list-value word-breakable">
                        <a href="<?php if (nbf_common::get_param($_REQUEST, 'attach_to_type') && nbf_common::get_param($_REQUEST, 'attach_to_id')){echo '#';} else {echo nbf_cms::$interop->admin_popup_page_prefix; ?>&action=supporting_docs&task=download&file=<?php echo base64_encode($current_path . "/" . $file['name']);} ?>" onclick="<?php if (nbf_common::get_param($_REQUEST, 'attach_to_type') && nbf_common::get_param($_REQUEST, 'attach_to_id')){ ?>document.adminForm.task.value='attach';document.adminForm.current_file.value='<?php echo $file['name']; ?>';if(window.opener.document.adminForm.task.value.length>0){window.opener.document.adminForm.task.value='edit';if(window.opener.document.adminForm.use_posted_values){window.opener.document.adminForm.use_posted_values.value='1';}}document.adminForm.submit();return false;<?php } else { ?>return true;<?php } ?>"><img style="vertical-align:middle" border="0" alt="" src="<?php echo nbf_cms::$interop->nbill_site_url_path; ?>/images/file.png" />&nbsp;<?php echo $file['name']; ?></a>
