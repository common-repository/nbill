	...
	public static function showInvoices(&$rows, &$pagination, &$vendors, $first_product_description, $cfg_date_format, $orders_present, &$page_totals, &$sum_totals, &$quote_status, $awaiting = null, $attachments = array())
	{
        ...
        <div class="rounded-table">
            ...
            <tr class="nbill-admin-title-row">
                ...
                switch ($feature) {
                    case 'QUOTES':
                        self::renderCustomColumn('quote_no');
                        break;
                    case 'CREDITS':
                        self::renderCustomColumn('credit_no');
                        break;
                    default:
                        self::renderCustomColumn('invoice_no');
                        break;
                }
                self::renderCustomColumn('document_no'); ?>
			    <th class="title">
		    ...
		    for ($i=0, $n=count( $rows ); $i < $n; $i++)
		    {
                ...
                switch ($feature) {
                    case 'QUOTES':
                        self::renderCustomColumn('quote_no', $row);
                        break;
                    case 'CREDITS':
                        self::renderCustomColumn('credit_no', $row);
                        break;
                    default:
	...
	public static function editInvoice($document_id, $row, $line_item_factory, $line_items, $vendors, $clients, $selected_client_row, $default_tax_rate, $client, $tax_rates, $countries, $currencies, $ledger, $payment_plans, $shipping, $shipping_price, $use_posted_values, $client_changed = false, $attachments = array(), $related_docs = array(), $scroll_to_items = false)
	{
        ...
        function refresh_editor(editorID, content)
        {
            if (document.getElementById(editorID)) {
                ...
                if (document.getElementById(editorID).value.replace(/(\r\n|\n|\r)/gm, "") != content.replace(/(\r\n|\n|\r)/gm, ""))
                {
                    ...
                    { ?>
                        if (div_id == 'div_detailed_desc') {
                            ...
                            var prev_style_display = document.getElementById(div_id).style.display;
                            ...
                            if (editor_doc)
                            {
                                ...
                                if (document.getElementById(div_id) && document.getElementById(div_id).style && prev_style_display) {
                                    document.getElementById(div_id).style.display = prev_style_display;
                                }
                            }
                            else if (!is_codemirror)
                            {
                                ...
                                {
                                    setTimeout(function(){jInsertEditorText(content, editorID);if (document.getElementById(div_id) && document.getElementById(div_id).style && prev_style_display) { document.getElementById(div_id).style.display = prev_style_display;}}, 50); //ckeditor needs this to run async
                                }
                                else
                                {
                                    if (editor_doc)
