...
function format_number($float, $decimal_places = null, $format_negative = false, $suppress_commas = true, $negative_in_brackets = null, $currency = '')
{
    ...
    if (substr($decimal_places, 0, 9) == 'editable_' || $suppress_commas) {
        $number = nbf_common::convertValueToNumberObject($float);
        $number = $number->getEditableDecimal();
    } else if ($decimal_places === null) {
        $number = nbf_common::convertValueToNumberObject($float);
    } else if ($decimal_places == 'currency_line' || $decimal_places == 'editable_currency_line') {
        if ($currency) {
    ...
    } else if ($decimal_places == 'currency_grand' || $decimal_places == 'editable_currency_grand') {
        if ($currency) {
            $number = nbf_common::convertValueToCurrencyObject($float, $currency, true);
        } else {
            $decimal_places = $config->precision_currency_grand_total;
        }
    } else if ($decimal_places == 'currency' || $decimal_places == 'editable_currency') {
        if ($currency) {
...
function get_precision_value($precision)
{
    ...
    } else if ($precision == 'currency_line' || $precision == 'editable_currency_line') {
        $precision = $config->precision_currency_line_total;
    } else if ($precision == 'currency_grand' || $precision == 'editable_currency_grand') {
        $precision = $config->precision_currency_grand_total;
    } else if ($precision == 'currency' || $precision == 'editable_currency') {
