...
class nbf_common
{
    ...
    */
    ...
    function nb_password_encrypt($plain_text)
    {
        $nb_database = nbf_cms::$interop->database;
        $sql = "SELECT site_id_hash FROM #__nbill_license WHERE id = 1";
        $nb_database->setQuery($sql);
        $site_id_hash = $nb_database->loadResult();
        $salt1 = strtoupper(substr(md5(uniqid("X", true)), 0, 19));
        $salt2 = strtoupper(substr(sha1(uniqid("A")), 0, 11));

        $password = sha1($salt2 . $site_id_hash . $salt1);

        //Use MySQL AES functions to encrypt (maybe resource intensive but will not happen often)
        $sql = "UPDATE #__nbill_license SET temp_data = HEX(AES_ENCRYPT('" . $plain_text . "','" . $password . "')) WHERE id = 1";
        $nb_database->setQuery($sql);
        $nb_database->query();
        $sql = "SELECT temp_data FROM #__nbill_license WHERE id = 1";
        $nb_database->setQuery($sql);
        $encrypted = $nb_database->loadResult();
        $sql = "UPDATE #__nbill_license SET temp_data = NULL WHERE id = 1";
        $nb_database->setQuery($sql);
        $nb_database->query();

        $encrypted = $salt1 . $encrypted . $salt2;
        $encrypted = $encrypted . ":" . substr(md5(str_rot13($encrypted)), 13, 5);
        return $encrypted;
    }

    function nb_password_decrypt($encrypted_text)
    {
        //Check hash
        $hash_pos = strrpos($encrypted_text, ":");
        if ($hash_pos !== false)
        {
            $hash = substr($encrypted_text, $hash_pos + 1);
            $data = substr($encrypted_text, 0, $hash_pos);
            if (substr(md5(str_rot13($data)), 13, 5) == $hash)
            {
                //Extract salt
                $salt1 = substr($data, 0, 19);
                $salt2 = substr($data, strlen($data) - 11);
                $data = substr($data, 19, strlen($data) - 30);
                ...
                //Work out password for decryption
                $password = sha1($salt2 . $site_id_hash . $salt1);

                //Do the decrypt
                $sql = "UPDATE #__nbill_license SET temp_data = '" . $nb_database->getEscaped($data) . "' WHERE id = 1";
                $nb_database->setQuery($sql);
                $nb_database->query();
                $sql = "SELECT AES_DECRYPT(UNHEX(temp_data), '" . $password . "') FROM #__nbill_license WHERE id = 1";
                $nb_database->setQuery($sql);
                $decrypted = $nb_database->loadResult();
                return $decrypted ? $decrypted : $encrypted_text;
            }
        }
