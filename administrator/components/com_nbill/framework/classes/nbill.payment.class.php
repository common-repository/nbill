...
///RSW TODO: When we are storing ledger assignment for partial payments against the document item, we will need to adjust this to deduct only the amount assigned for that ledger/document_item (currently we assume the whole amount was paid)
    ...
    */
    ...
    {
        $nb_database = nbf_cms::$interop->database;
        $sql = "SELECT #__nbill_document.total_gross - COALESCE(SUM(#__nbill_document_transaction.gross_amount),0)
                FROM #__nbill_document
                LEFT JOIN #__nbill_document_transaction ON #__nbill_document_transaction.document_id = #__nbill_document.id
        ...
        if (intval($details['client_id'])) {
            ...
            if (!$details['client_email']) {
                $sql = "SELECT #__nbill_contact.email_address
                    ...
                    INNER JOIN #__nbill_entity_contact ON #__nbill_entity_contact.contact_id = #__nbill_contact.id
                    WHERE #__nbill_entity_contact.entity_id = " . intval($details['client_id']) . "
                    AND #__nbill_contact.email_address != ''
