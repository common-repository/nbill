...
3.2.2 (27/10/2015)
Fixed an issue checking the cron authorisation token after upgrading from 2.6.1

3.2.1 (22/10/2015)
Fixed an issue where orders were not being created if pending until paid was set to no
Fixed an issue where confirmation e-mail was not being sent if an error occurred during order creation

3.2.0 (15/10/2015)
Fixed race condition when attaching supporting documents to records so that popup window can always be closed and attached files always show up on auto page refresh
Upgrade from nBill Lite 2.x now provides a default value for form_id column on orders table (to prevent error when database server is running in strict mode)
Fixed issue with double quote characters in invoice line items
E-mail validation updated to allow TLDs longer than 4 characters
Fix to prevent duplicate order creation when initial payment and recurring amount differ and simultaneous webhooks are sent by GoCardless
